...
def main():
    ...
    ap.add_argument("--meta", default=None,
                    help="sample_meta.csv-style catalogue; each row is one "
                         ...
                         "uses the path/subject/side stored in the CSV so raw "
                         "files (timestamp-only names, hand in parent dir) "
                         "are processed correctly.")
    ap.add_argument("--recursive", action="store_true",
                    help="recursive glob under input_dir (for raw data laid "
                         "out as {subject}/{date-hand}/{timestamp}.npz)")
    ap.add_argument("--allow-force", action="store_true",
                    help="include /mnt/force_data/ recordings. Force-task data "
                         "is isometric (steady contraction, little pose change)"
                         " and produces near-noise clips, so it is skipped by "
                         "default to keep the ground-truth set clean.")
    ...
    # Build the work list. --meta drives the iteration when set (one row per
    # recording, with hand baked in -- "both"-side rows expand to two passes).
    # Otherwise scan input_dir for .npz files.
    if args.meta:
        work = [(info.path, info)
    ...
    if not work:
        print(f"No .npz files found (input_dir={args.input_dir}, meta={args.meta})")
        return

    # Filter out force_data unless explicitly allowed -- reference deliberately
    # excludes it ([reference/gesture_velocity_segmentation.ipynb] keeps only
    # /mnt/pose_data/), and isometric force tasks contain near-zero gesture
    # motion so pose-speed segmentation produces noise.
    ...
    for path, info in work:
        ...
        try:
            ...
            # Raw session dirs commonly contain non-sample npz (norm_stats.npz
            # etc.) and broken recordings -- skip rather than abort a long run.
        ...
        # Apex of each clip = max joint deviation across its static_out (the
        # held pose after the gesture's motion), same definition as the EMG
        # path so a labelled clip and its matched burst point at the same
        # frame.
        clip_apexes = []
        for c in clips:
        ...
        # --- Cross-reference (each side knows about the other) -------------
        ...
        # Raw Manus drops frames during occlusion; high pose_nan_frac means
        # the recording is largely fabricated by interpolation and should
        # not be trusted as ground truth.
    ...
    print(f"Wrote {seg_csv} ({len(seg_rows)} burst segments)")

    clip_csv = os.path.join(cfg.out_dir, "clips.csv")
    ...
    print(f"Wrote {clip_csv} ({len(clip_rows)} clips -- labelling unit)")

    rec_csv = os.path.join(cfg.out_dir, "recordings.csv")
    ...
    n_lag_ok = sum(1 for r in rec_rows if r["lag_flag"] == "ok")
    print(f"Wrote {rec_csv} ({len(rec_rows)} recordings, "
          f"{n_lag_ok}/{len(rec_rows)} with physiological EMG-pose lag)")
